...
- This repository serves as the source for building GitHub Actions runner images for Ubuntu. You can find exact versions in the [Available Images](../README.md#available-images) section of README.md. Ubuntu images build on Hetzner Cloud infrastructure using Packer.
...
- **What this repo builds:** the source used to generate the VM images for [GitHub-hosted runners](https://docs.github.com/en/actions/using-github-hosted-runners/about-github-hosted-runners) (Actions). See [README.md](../README.md#about).
- **Platforms and labels:** Ubuntu across multiple versions and architectures (x64/arm64). See [Available Images](../README.md#available-images) for the supported YAML labels.
- **Build systems:**
  - Ubuntu images are built with [Packer](https://www.packer.io/) (HCL2 templates) on **Hetzner Cloud**.
- **Build flow:** a Packer template provisions a temporary Hetzner Cloud VM, runs install scripts in order, runs validation tests, captures a managed image, then generates the image's software report. A failed step aborts the build and cleans up resources. Details: [docs/create-image-and-hcloud-resources.md](../docs/create-image-and-hcloud-resources.md).

## Repository map

| Path | What lives here |
| --- | --- |
| `images/<os>/` | Per-image sources, where `<os>` is a Packer-built VM image: `ubuntu`. (`ubuntu-slim` is Docker-based with a different layout — see its own row.) |
...
| `images/<os>/scripts/helpers/` | Shared install/validation helpers (e.g. Ubuntu `install.sh`). |
...
| `docs/` | Build and topic docs (start with `create-image-and-hcloud-resources.md`). |
...
- **Adding or updating a tool** (the most common change) requires, together:
  ...
  2. a Pester validation test in `images/<os>/scripts/tests/`, registered from the install script — `invoke_tests "<TestFile>" "<TestName>"` in Bash scripts (Ubuntu),
...
- Focus on re-using helpers when writing scripts. Linux and Ubuntu scripts have helper functions available to simplify installation and validation.
...
- Audience: Open-source contributors, GitHub Actions maintainers, and developers building custom runner images. Assume familiarity with CI/CD concepts, Packer, and basic infrastructure provisioning, but explain platform-specific details (Hetzner Cloud for Ubuntu) when relevant.
...
- Do not expose sensitive credentials (API tokens, etc.) in code examples.
